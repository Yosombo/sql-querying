...
-- TEST______ (app_name, category, rating, reviews, size, min_installs, price, content_rating, genres, last_updated, current_version, android_version)
...
SELECT app_name, reviews FROM analytics WHERE price BETWEEN 0.10 AND 1.00;
...
SELECT app_name, last_updated FROM analytics ORDER BY last_updated LIMIT 10;
...
SELECT category, COUNT(category) as total FROM analytics GROUP BY category HAVING COUNT(category) > 300; 
